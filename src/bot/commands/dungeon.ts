...
import { WEBAPP_BASE_URL, WEBAPP_PATHS, DUNGEON_MIN_LEVEL } from "../../config/constants";
...
export async function handleDungeonCommand(ctx: Context, db: D1): Promise<void> {
  if (!isGroupChat(ctx)) {
  ...
  if (player && player.level < DUNGEON_MIN_LEVEL) {
    await ctx.reply(`این بخش از لول ${DUNGEON_MIN_LEVEL} به بالا باز می‌شود. لول فعلی شما: ${player.level}`);
    return;
  }
  if (player?.dungeonCooldownUntil && player.dungeonCooldownUntil > Date.now()) {
    await ctx.reply(`شما اخیراً باخته‌اید. ${formatRemaining(player.dungeonCooldownUntil - Date.now())} دیگر صبر کنید.`);
    return;
  }
