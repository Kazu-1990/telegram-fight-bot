...
کاتالوگ آدرس عکس‌ها داخل خودِ فایل‌های `src/webapp/duel/index.html` و `src/webapp/pve/index.html`
هست (اول فایل، توی یه بخش `<script>` با کامنت مشخص «کاتالوگ گرافیک» - دنبال متغیر
`RACE_ASSETS` بگرد). چون هر دو مینی‌اپ به این کاتالوگ نیاز دارن، توی هر دو فایل یه کپی ازش هست؛
اگه یه مسیر عکس رو عوض کردی، توی هر دو فایل عوضش کن.

مثال - به‌جای مسیر فعلی (مثل `/assets/characters/werewolf/idle_1.png`)، لینک مستقیم واقعی رو بذار:
```
...
(دو روش قبلی که امتحان کردیم و کنار گذاشته شدن چون قابل‌اعتماد نبودن: سرو کردن از پوشه‌ی `public`،
و ایمپورت جدا کردن فایل‌های js/css با rule توی wrangler.toml. اگه جایی به این‌ها اشاره شده بود، دیگه
معتبر نیست.)
